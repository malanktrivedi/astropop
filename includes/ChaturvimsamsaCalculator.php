...
/**
 * Classical Parashari D24 (Chaturvimsamsa / Siddhamsa) calculator.
 * Each 30° Rashi is divided into twenty-four 1°15′ parts.
 * Odd signs start from Leo; even signs start from Cancer.
 * The twelve Siddhamsa deities repeat twice; even signs use reverse order.
 */
final class ChaturvimsamsaCalculator
{
    private const SIGNS = ['Aries','Taurus','Gemini','Cancer','Leo','Virgo','Libra','Scorpio','Sagittarius','Capricorn','Aquarius','Pisces'];
    private const PART_SIZE = 1.25;
    private const DEITIES = [
        'Skanda','Parashudhara','Anala','Vishvakarma','Bhaga','Mitra',
        'Maya','Antaka','Vrishadhvaja','Govinda','Madana','Bhima'
    ];
    ...
    public function calculate(array $planets, array $chartData, ?string $d1Lagna): array
    {
        ...
        $d24Lagna = $ascDegree !== null ? $this->chaturvimsamsaSign($ascDegree) : null;
        ...
        foreach ($planets as $planet) {
            ...
            $d24Sign = $this->chaturvimsamsaSign($global);
            ...
            $positions[] = [
                ...
                'part' => $part,
                'd24_sign' => $d24Sign,
                'd24_degree' => $this->chaturvimsamsaDegree($global),
        ...
        $houses = [];
        if ($d24Lagna !== null) {
            $lagnaNo = $this->signNumber($d24Lagna);
            foreach ($positions as &$position) {
                $signNo = $this->signNumber((string) $position['d24_sign']);
        ...
        return ['lagna' => $d24Lagna, 'positions' => $positions, 'houses' => $houses];
    }

    private function chaturvimsamsaSign(float $longitude): string
    {
        $degree = fmod($longitude, 360.0);
        if ($degree < 0) $degree += 360.0;
        $d1SignNo = (int) floor($degree / 30.0) + 1;
        $within = $degree - (($d1SignNo - 1) * 30.0);
        $part = min(23, (int) floor(($within + 1e-10) / self::PART_SIZE));
    ...
    private function startSign(int $signNo): int
    {
        ...
        return in_array($signNo, [1,3,5,7,9,11], true) ? 5 : 4;
    }
    ...
    private function partNumber(float $longitude): int
    {
        ...
        return min(24, (int) floor(($degree + 1e-10) / self::PART_SIZE) + 1);
    }

    private function chaturvimsamsaDegree(float $longitude): float
    {
    ...
    {
        $sequence = in_array($signNo, [1,3,5,7,9,11], true)
            ? self::DEITIES
            : array_reverse(self::DEITIES);
        return $sequence[($part - 1) % 12];
    }
