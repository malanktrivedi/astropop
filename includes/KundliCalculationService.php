...
final class KundliCalculationService
{
    ...
    /**
     * Calculate the source astrology data once and persist the canonical
     * calculation. Report pages must consume this stored record instead.
     *
     * @param array<string,mixed> $profile
     * @return array{ok:bool,calculation_id:int|null,hash:string|null,warnings:list<string>,error:string|null}
     */
    public function calculateAndStore(int $userId, array $profile): array
    {
        ...
        if (count($planetary) < 5 || !is_array($chart['ascendant'] ?? null)) {
            return ['ok'=>false,'calculation_id'=>null,'hash'=>$hash,'warnings'=>[],'error'=>'The astrology service returned incomplete canonical chart data.'];
        }
        ...
        if ($dashaResult['ok']) {
            ...
            if (is_array($response) && !empty($response['mahadasha']) && !empty($response['mahadasha_order'])) {
                ...
                $warnings[] = 'Maha Dasha response was returned but did not contain the expected timeline.';
            }
        } else {
            $warnings[] = 'Maha Dasha could not be cached: ' . (string) $dashaResult['error'];
        }

        $apiResponse = [
            'planet_details' => $planet['data'] ?? [],
            'ascendant_report' => $ascendant['data'] ?? [],
        ];
        ...
        if ($existing) {
            ...
            $stmt->bind_param('ssssssssssii', $lagna, $rashi, $nakshatra, $planetaryJson, $houseJson, $dashaJson, $chartJson, $apiJson, self::ENGINE_VERSION, self::API_VERSION, $id, $userId);
        } else {
            ...
            $stmt->bind_param('iisssssssssss', $userId, $profileId, $lagna, $rashi, $nakshatra, $planetaryJson, $houseJson, $dashaJson, $chartJson, $apiJson, $hash, $engineVersion = self::ENGINE_VERSION, $apiVersion = self::API_VERSION);
        }
