...
/* Match the Yoga report to the same birth profile used by the dashboard/Kundli
 * links. Never fall back to an unrelated latest calculation when no profile_id
 * is supplied. */
...
$calc = null;
if ($requestedProfileId > 0) {
    $stmt = db()->prepare('SELECT p.id,p.profile_name,p.full_name,p.location_name,p.birth_place,c.id AS calculation_id,c.lagna,c.rashi,c.nakshatra,c.planetary_data,c.chart_data,c.api_response FROM birth_profiles p LEFT JOIN kundli_calculations c ON c.birth_profile_id = p.id AND c.user_id = ? WHERE p.id = ? AND p.user_id = ? ORDER BY c.id DESC LIMIT 1');
...
/* Older cached calculations can have incomplete normalized fields. Rebuild the
 * normalized chart from the original API payload when necessary. */
$hasMoon = false;
$hasJupiter = false;
foreach ($planetary as $p) {
    if (!is_array($p)) continue;
    $name = strtolower(trim((string) ($p['name'] ?? $p['full_name'] ?? '')));
    if ($name === 'moon') $hasMoon = true;
    if ($name === 'jupiter') $hasJupiter = true;
}
...
if ((!$hasMoon || !$hasJupiter || empty($chartData['ascendant'])) && ($rawPlanetDetails || $rawAscendant)) {
...
<?php if($analysisError!==null): ?><div class="alert alert-error"><strong>Yoga analysis error:</strong> <?=yg($analysisError)?><br><span class="muted">The chart data is intact; this message identifies the calculation-layer failure.</span></div><?php endif; ?>
